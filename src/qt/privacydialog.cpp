...
#include "zglobalsmartassetcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zsnd ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzsnd->setToolTip(tr("zsnd is currently disabled due to maintenance."));

        ui->pushButtonSpendzsnd->setEnabled(false);
        ui->pushButtonSpendzsnd->setToolTip(tr("zsnd is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzsnd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zsnd is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zsnd...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zsnd in ") +
...
void PrivacyDialog::on_pushButtonSpendzsnd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zsnd is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zsnd
        sendzsnd();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zsnd
    sendzsnd();
}

void PrivacyDialog::on_pushButtonZglobalsmartassetControl_clicked()
{
    ZglobalsmartassetControlDialog* zsndControl = new ZglobalsmartassetControlDialog(this);
    zsndControl->setModel(walletModel);
    zsndControl->exec();
}

void PrivacyDialog::setZglobalsmartassetControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzsnd()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid globalsmartasset Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zsnd is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " globalsmartasset </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zsnd</b>";
    ...
    // use mints from zsnd selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZglobalsmartassetControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZglobalsmartassetControlDialog::GetSelectedMints();
    }
    ...
    // Clear zglobalsmartasset selector in case it was used
    ZglobalsmartassetControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zsnd Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " globalsmartasset, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zsnd Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zsnd </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zsnd "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zsnd "));
