...
#include "ui_zglobalsmartassetcontroldialog.h"
...
std::list<std::string> ZglobalsmartassetControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZglobalsmartassetControlDialog::listMints;

ZglobalsmartassetControlDialog::ZglobalsmartassetControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZglobalsmartassetControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZglobalsmartassetControlDialog::~ZglobalsmartassetControlDialog()
{
    delete ui;
}

void ZglobalsmartassetControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZglobalsmartassetControlDialog::updateList()
{
...
void ZglobalsmartassetControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZglobalsmartassetControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZglobalsmartasset_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZglobalsmartassetControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZglobalsmartassetControlDialog::GetSelectedMints()
{
...
void ZglobalsmartassetControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
