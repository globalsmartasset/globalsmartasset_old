...
#include "config/globalsmartasset-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("globalsmartasset-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "globalsmartasset.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "globalsmartassetd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the globalsmartasset and zsnd money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 22711, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in globalsmartasset/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in globalsmartasset/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, globalsmartasset, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in globalsmartasset/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all globalsmartasset specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:22711"));
    ...
    strUsage += HelpMessageOpt("-backupzglobalsmartasset=<n>", strprintf(_("Enable automatic wallet backups triggered after each zsnd minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeglobalsmartassetamount=<n>     " + strprintf(_("Keep N globalsmartasset anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The globalsmartasset Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("globalsmartasset-loadblk");
...
/** Sanity checks
 *  Ensure that globalsmartasset is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. globalsmartasset Core is shutting down."));
...
#endif
    // Make sure only a single globalsmartasset process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. globalsmartasset Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("globalsmartasset version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // globalsmartasset: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZglobalsmartassetMinted();
                        RecalculateZglobalsmartassetSpent();
                    }
                    RecalculateglobalsmartassetSupply(1);
                }
                ...
                // globalsmartasset: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of globalsmartasset Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart globalsmartasset Core to complete") << "\n";
        ...
        bool fEnableZglobalsmartassetBackups = GetBoolArg("-backupzglobalsmartasset", true);
        pwalletMain->setZglobalsmartassetAutoBackups(fEnableZglobalsmartassetBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeglobalsmartassetAmount = 2;
...
//    }
//
//    nAnonymizeglobalsmartassetAmount = GetArg("-anonymizeglobalsmartassetamount", 0);
//    if (nAnonymizeglobalsmartassetAmount > 999999) nAnonymizeglobalsmartassetAmount = 999999;
//    if (nAnonymizeglobalsmartassetAmount < 2) nAnonymizeglobalsmartassetAmount = 2;
    ...
    LogPrintf("Anonymize globalsmartasset Amount %d\n", nAnonymizeglobalsmartassetAmount);
    ...
    /* Denominations
       ...
       For example:
       1globalsmartasset+1000 == (.1globalsmartasset+100)*10
       10globalsmartasset+10000 == (1globalsmartasset+1000)*10
    */
